...
<%
	...
	List<BoardDTO> boardList_re = (List<BoardDTO>)session.getAttribute("boardList_re");
...
%>
	...
	document.addEventListener("DOMContentLoaded", function(){
	    ...
	    if('<%=memberId%>' == '<%=writer%>') {
			...
			btn_boardList.addEventListener("click", function() {
				location = 'boardList.do?pageNum=' + pageNum;
			});
	...
	<div id="container">
		<form action="boardWriteForm.jsp" method="post" name="contentForm">
			...
			<div class="info">
				<h2 id><%=board.getTitle()%></h2>
			</div>
			<div class='info'>
				<span id="info_top"><%=board.getWriter()%></span> <br> 
				<span id="info_bottom"><%=sdf.format(board.getRegDate()) %> 조회수 <%=board.getReadCount()%></span>  
			</div>
			<hr>
			<div id="content_box">
				<%=board.getContent()%>
			...
			<%for (BoardDTO boardL : boardList_re) {
				if ((ref == boardL.getRef()) && (boardL.getRe_step() > 0)) {
					int num1 = boardL.getSeq();
